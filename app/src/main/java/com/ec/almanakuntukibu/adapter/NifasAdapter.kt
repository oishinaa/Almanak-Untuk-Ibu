...
class NifasAdapter(context: NifasActivity, private var items: ArrayList<VisitModel>, private var hl: Calendar) :
    ...
    private fun submitPassword(text: String, id: Int, visit_date: Int, visit_time: String, visit_notes: String, visit_status: Boolean) {
        if (text == "nganjukbangkit") {
            ...
            builder.setView(dialogLayout)
            builder.setPositiveButton("Oke") { _,_ -> db.updVisit(id,2, dbFormatter.format(date.time).toInt(), tmFormatter.format(date.time), edtNotes.text.toString(), if (chkStatus.isChecked) 1 else 0); restartActivity() }
            builder.setNegativeButton("Batal") { _,_ -> }
    ...
    private fun restartActivity() {
        context.sendBroadcast(Intent("finish km"))
        context.startActivity(Intent(context, NifasActivity::class.java))
    }
    ...
    private fun showDatePickerDialog(onDateSetListener: DatePickerDialog.OnDateSetListener) {
        ...
        datePickerDialog.datePicker.minDate = hl.timeInMillis
