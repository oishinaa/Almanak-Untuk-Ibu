...
class SiklusAdapter(context: Context, days: ArrayList<Date>, private val current: Calendar, private val cycles: ArrayList<CycleModel>, private val totalCyclesAfterRes: Int, private val isPreg: Boolean) :
    ArrayAdapter<Date>(context, R.layout.item_tanggal, days) {
    ...
    override fun getView(position: Int, view: View?, parent: ViewGroup): View {
        ...
        linearLayout = view ?: inflater.inflate(R.layout.item_tanggal, parent, false)
    ...
    private fun loadCycles(date: Date) {
        if (cycles.size > 0) {
            ...
            for ((i, cycle) in cycles.withIndex()) {
                ...
                clndSta.set(getDatePart("yyyy", dateSta!!), getDatePart("MM", dateSta)-1, getDatePart("dd", dateSta), 0, 0, 0)
                clndEnd.set(getDatePart("yyyy", dateEnd!!), getDatePart("MM", dateEnd)-1, getDatePart("dd", dateEnd), 0, 0, 0)
                ...
                var avgDiff = 28
                if (i > 0 && i > totalCyclesAfterRes-3 && totalCyclesAfterRes-3 >= 0) {
                    count++
                ...
                if (i+1 == cycles.size && !isPreg && totalCyclesAfterRes > 0) {
                    val nextCycle = Calendar.getInstance()
                    nextCycle.set(getDatePart("yyyy", dateSta), getDatePart("MM", dateSta)-1, getDatePart("dd", dateSta), 7, 0, 0)
                    nextCycle.add(Calendar.DATE, avgDiff)
                    if (alarm == 0) {
                        AlarmUtils(context).setAlarm(nextCycle, "1", "Jangan lupa untuk mendata siklus haid anda.")
                        alarm = 1
                    }
    ...
    private fun addCycleDialog(date: Date) {
        ...
        tempEnd.set(end.get(Calendar.YEAR), end.get(Calendar.MONTH), end.get(Calendar.DATE), 0, 0, 0)
        ...
        for (cycle in cycles) {
            if (cycle.sta <= dbFormatter.format(tempEnd.time).toInt() && cycle.sta >= dbFormatter.format(sta.time).toInt()) {
                ...
                end.set(getDatePart("yyyy", cycleSta!!), getDatePart("MM", cycleSta)-1, getDatePart("dd", cycleSta), 0, 0, 0)
    ...
    private fun updCycleDialog(cycleId: Int, cycleSta: Date, cycleEnd: Date) {
        ...
        lnlSta.setOnClickListener { showDatePickerDialog(onStaSetListener, "sta") }
        sta.set(getDatePart("yyyy", cycleSta), getDatePart("MM", cycleSta)-1, getDatePart("dd", cycleSta), 0, 0, 0)
        lnlEnd.setOnClickListener { showDatePickerDialog(onEndSetListener, "end") }
        end.set(getDatePart("yyyy", cycleEnd), getDatePart("MM", cycleEnd)-1, getDatePart("dd", cycleEnd), 0, 0, 0)
        ...
        if (type == "sta") {
            for (cycle in cycles) {
                if (cycle.end < dbFormatter.format(sta.time).toInt()) {
                    ...
                    minDate.set(getDatePart("yyyy", cycleEnd!!), getDatePart("MM", cycleEnd) - 1, getDatePart("dd", cycleEnd), 0, 0, 0)
            ...
            for (cycle in cycles) {
                if (cycle.sta > dbFormatter.format(end.time).toInt()) {
                    ...
                    maxDate.set(getDatePart("yyyy", cycleSta!!), getDatePart("MM", cycleSta) - 1, getDatePart("dd", cycleSta), 0, 0, 0)
